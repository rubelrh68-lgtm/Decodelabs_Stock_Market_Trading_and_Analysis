...
**Period:** 1 January 2024 – 31 May 2026 (609 daily candles per ticker)
...
1. **Raw OHLC interpretation** — candle anatomy, descriptive statistics, return distributions, volume, correlation
2. **Wick-to-body ratio (R_wb)** — quantifies price rejection per candle: (upper wick + lower wick) / body
3. **Support, resistance & trendlines** — pivot detection (10-day window), level clustering, linear-regression trend channels
4. **EMA 50/200 indicators** — regime detection, Golden Cross / Death Cross, ATR-14 volatility
5. **RSI momentum filter** — Wilder RSI(14), 50-threshold crossovers, divergence detection
6. **IPO Decision Engine** — the three-gate confluence logic described above
7. **Trade levels & risk management** — ATR-based stop-loss/take-profit (1.5x / 3.0x, R:R 2.0), an 80/20 train/test backtest, and a three-protocol circuit-breaker system (15-minute hard stop, half-size rule, session-end trigger)
...
- This is a **technical analysis project only** — no fundamental valuation, earnings, or macro factors are considered. Signals should be cross-referenced with fundamental analysis (see [Project 2](../Project2_Fundamental_Valuation)) before any capital is committed.
