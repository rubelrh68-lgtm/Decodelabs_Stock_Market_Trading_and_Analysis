...
These results are a single-strategy snapshot, not a guarantee: backtest performance is sensitive to the exact indicator methodology used, the underlying fundamental valuation assumptions inherited from Project 2, and the absence of transaction costs. See [Limitations](#limitations) below.
